...
def is_committer(repo_name, committer_name):
    token = os.getenv('GITHUB_TOKEN')
    org = os.getenv('GITHUB_ORGANIZATION')
    headers = {
        'Authorization': f'Bearer {token}',
    ...
    # Fetch commits and check if the committer is involved
    response = requests.get(f'https://api.github.com/repos/{org}/{repo_name}/commits', headers=headers)

    if response.status_code == 200:
        commits = response.json()
        for commit in commits:
            if commit['commit']['author']['name'] == committer_name:
                return True
    return False
...
def update_readme_with_repos():
    ...
    if response.status_code == 200:
        ...
        for repo in repos:
            ...
            is_committer = is_committer(repo_name, committer_name)
            if is_contrib or is_committer:
